...
#define BUTTON_PIN 17   // Push button input pin
...
Debounce motorButton(BUTTON_PIN, HIGH);
...
const float MAX_RPM = 5000.0;  // Maximum motor RPM
...
// Add the help function definition
void displayHelp()
{
  ...
  Serial.println(F("A<ang>  - Set launch angle (0-90°)"));
  ...
  Serial.println(F("A10     - Set angle to 10°"));
...
void loop() {
    ...
    if (millis() - lastDebounceTime >= DEBOUNCE_INTERVAL) {
        ...
        motorButton.update(); // Update button state every 1ms
    }
    ...
    if (commandComplete)
    {
        ...
        if (command.length() > 0)
        {
            ...
            case 'S': // Stop
                // Reset encoder position when stopping
                encoderPosition = 0;
                lastPosition = 0;

                digitalWrite(IN1, LOW);    // Stop motor
                digitalWrite(IN2, LOW);    // Stop motor
                ledcWrite(PWM_CHANNEL, 0); // Set PWM to 0% duty cycle
    ...
    // Modify the button handler section at the end of loop():
    if (motorButton.isPressed()) {
        motorRunning = !motorRunning; // Toggle motor state
        motor2Running = false;        // Always stop motor 2 when button pressed
